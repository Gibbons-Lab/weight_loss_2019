...
```{r}
gmap <- c(control = "no weight loss", `weight loss` = "successful weight loss")
...
cohort <- list(
    ...
    loss = fread("successful_weight_loss.csv", colClasses = classes)[, "group" := "successful weight loss"]
) %>% rbindlist()
...
ggplot(norm[state == "before" & `gene name` %in% sig], aes(x=group, y=delta)) + 
    ...
    theme(axis.text.x = element_text(angle = 30, vjust = 1, hjust=1))
ggsave("figures/deltas.png", dpi=300, width=6, height=4)
```
