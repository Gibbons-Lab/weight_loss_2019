...
We will also add in the rough BRITE class for each hit.

```{r}
library(stringr)
library(KEGGREST)

annotate <- function(ko) {
    res <- tryCatch(
        keggGet(ko)[[1]],
        error = function(e) return(NULL)
    )
    if (is.null(res)) {
        return(list(name = NA, brite = NA))
    }
    return(list(name = res$NAME, brite = paste0(res$BRITE, collapse="\n")))
}

tests[variable == "subset" & padj < 0.05, c("name", "brite") := annotate(ko_term), by=ko_term]
```
...
ggplot(tests[variable == "subset" & padj<0.05, ], 
       aes(x=log2FoldChange, y=name, color=log10(baseMean), group=ko_term)) +
