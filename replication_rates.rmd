...
ggsave("figures/replication.png", width=4, height=4, dpi=300)
```

```{r, fig.width=9, fig.height=1.5}
ggplot(rates, aes(x = subset, y = rate, color = phylum)) + 
    scale_y_log10() +
    geom_jitter( width=0.3) + facet_wrap(~ phylum, nrow=1) + coord_flip() +
    labs(x = "", y = "replication rate [a.u.]") + guides(color = FALSE)
```

```{r}
glm(log(rate) ~ age + bmi + subset, data=rates[phylum == "Verrucomicrobia"]) %>% summary()
```
